...
const AddProject: FC = () => {
  ...
  function handleNext() {
    ...
    next();
    setActiveHeader((prev) => [...prev, addProjectHeader[currentStep + 1].title]);
    setCompletedHeader((prev) => [
      ...prev,
      addProjectHeader[currentStep === 0 ? currentStep : currentStep - 1].title,
    ]);
  }
  ...
  return (
    <div className=" container flex gap-10 mb-20">
            ...
            <div className="flex flex-col gap-1  items-center">
              <div
                className={cn(
                  ...
                  activeHeader.includes(header.title) && "bg-primary_blue/40",
                  completedHeader.includes(header.title) && "bg-primary_blue"
                )}
              >
              ...
              { index != (addProjectHeader.length - 1) &&
                <Separator
                  className={cn(
      ...
      <div className="flex flex-1 flex-col w-full gap-[30px] max-w-[500px] ">
        <h1 className=" font-Jakarta font-semibold text-xl text-dark_green">
          {activeHeader.length === 1
            ? "Add project"
            : activeHeader.length === 2
            ? "Add project details"
            : activeHeader.length === 3
            ? "Preview project"
            : "Add project"}
        </h1>
        {step}
