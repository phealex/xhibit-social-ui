        ...
        />
        {
          ...
          render={({ field }) => (
            <FormItem className=" w-full flex flex-col gap-[2px]">
              <FormLabel className=" font-Jakarta font-medium text-base text-dark_green">
                {
                    form.getValues().idType === 'passport' ? 'Passport number' : form.getValues().idType === 'nin' ? 'National Identity Number' : form.getValues().idType === 'drivers' ? 'Driver\'s License number' : form.getValues().idType === "voters" ?  'Voter\'s Card number' : "ID number"
                }
              </FormLabel>
              <FormControl>
                <Input
